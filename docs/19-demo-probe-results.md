...
| Doc | Change |
|---|---|
| `04-erp-sync.md` | Back-link mechanism decided (WebExcellentAPI `getrecordlinks`, not `RLinkVc` REST parsing); WebExcellentAPI document-missing failure shape documented; create-push must verify persistence, not just absence of `<error>`; **`WONr` resolved 2026-07-07** (`= -1` on create, no `WOVc` chain — owner + `PasteSVOInWS`); full `WSVc` + `SVOVc` creation field-mapping added; **`SVOVc` create RESOLVED** (§10 — live create succeeded as `SerNr 230022` after the number series was fixed; POST with no `SerNr`, REST auto-assigns); `Invoiced` mapping corrected to a linked `IVVc` (not `InvFlag`); `ItemType` integer write confirmed |
