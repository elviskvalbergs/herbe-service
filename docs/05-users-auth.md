...
Status: v0.5 (2026-07-07) — PIN/biometric unlock extends the device session token rather than forcing a full re-auth; token revocation on scope-exit/device-deprovision purges local scoped data (`03-architecture.md`). Previous: v0.4 (2026-07-07) — consistency pass: version header corrected (the round-6 crew/lead content below landed 2026-07-06 without a bump), ERP link target fixed to the `UserVc` person code (`04-erp-sync.md` register table is the authority), role activation phases noted (back office moved to Phase 1 — owner 2026-07-07). Previous: v0.3 (2026-07-05) — spec-line merge: **role-shaped login** (magic link for office, PIN + biometrics on paired field devices) folded into the verified suite baseline (Auth.js v5 everywhere; Entra ID login exists in neither sibling today).
