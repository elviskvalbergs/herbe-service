...
Two levels, deliberately separate: the **tenant role** (what a person may do in general) and the **per-job lead** (elevation within a crew job — round 6: each crew member has their own worksheet, `02-data-model.md`). Any technician can be made *lead of a job* — that makes their worksheet the crew's **lead worksheet**, whose `Done` transition is normally what prompts the order's manual "job done" action. The lead does **not** collect a shared customer signature: each technician captures their own on-device worksheet confirmation on their own worksheet (when `require_worksheet_confirmation` is on), and whole-job customer acceptance is the separate order-level `OrderSignoff` (`02-data-model.md`, `08-suite-integration.md` §4). Being lead is not a role change and grants no authority over other members' own worksheets — each technician still owns their own status transitions.

| Role | Can |
|---|---|
| **Technician** | see own (and optionally team) bookings/worksheets; execute their own worksheet: parts from own van, own time/distance, checklists, photos, own on-device worksheet confirmation; as **job lead**: also triggers the order's "job done" action for the crew (no shared signature — whole-job acceptance is the order-level signoff); create service orders/customers/service items in the field (tenant-configurable); see own van stock; see service history; prices hidden/shown per tenant policy |
