...
- Project scaffolding, CI/CD with **TDD gates** (coverage thresholds, red-blocks-merge), environments; i18n scaffolding (next-intl, portal's 7-locale setup)
- **Test infrastructure** (`15-testing-strategy.md`): fake ERP server + fixture recorder, sync simulation harness, golden-fixture library — built before the features that need them; **owner arranges the dedicated test ERP** (§5 list: test company with/without WebExcellentAPI, planned version upgrade, reference devices)
