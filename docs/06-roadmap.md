...
8. **Work Order (`WOVc`) chain question, new 2026-07-06** (`19-demo-probe-results.md` §10): does any launch tenant require a Work Order step between Service Order and Work Sheet? The demo system's `WSVc` REST create hard-rejects a missing/zero `WONr`. Ask Excellent whether this is a per-tenant module setting; if any launch tenant has it on, the push-queue design (`04-erp-sync.md`) needs an added `WOVc`-creation step that isn't currently planned
9. **`SVOVc` REST-create blocker, new 2026-07-07**: outbound service-order creation has **never succeeded live** — the probe's `POST` returned HTTP 200 echoing the payload with no error field, yet nothing persisted (a silent "Jau reģistrēts" no-op; `19-demo-probe-results.md` §10). Creation may have to happen ERP-side, or via a dedicated endpoint — owner deciding. Until resolved this blocks the push-queue flow "service order created in app → pushed on creation" (`04-erp-sync.md`) and the outbound half of the Phase 0 push-design ADRs
