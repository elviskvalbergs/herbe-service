...
9. **`SVOVc` REST-create — diagnosed 2026-07-07, needs a confirming write test** (was a blocker): the probe's `POST` returned HTTP 200 with a silent "Jau reģistrēts" ("already registered") no-op, nothing persisted (`19-demo-probe-results.md` §10). HAL source (halocron) shows this is a **`SerNr` collision**, not a missing field — creates must POST with **no `SerNr`** and let the ERP's `NextSerNr` allocate against the register's number series (supplying `0`/a reused value → "already registered"). Phase-0 write test: POST without `SerNr` + confirm an `SVOVc` number series exists for the period and REST user (`04-erp-sync.md`). Not gating design — the create field-set and number-assignment rule are known; only a live write confirmation remains
