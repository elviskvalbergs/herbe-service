...
9. **`SVOVc` REST-create — narrowed to a per-tenant number-series fix (live-tested 2026-07-07)**: a create POST with **no `SerNr`** still returned "Already registered" with an empty `SerNr`, but every other field built correctly (customer block + row pricing derived server-side; `ItemType=2` read back as `Warranty`). So payload/field-set are validated; `NextSerNr` is handing out an already-used/blank number — the tenant's `SVOVc` **number series is misconfigured/behind the data**. Onboarding fix: set the series' next value above the current max and make it available to the REST user (`04-erp-sync.md`). Decisive follow-up (retry with explicit unused `SerNr`) pending — distinguishes "repair series" from "adapter must supply the number". **Not a design blocker**
