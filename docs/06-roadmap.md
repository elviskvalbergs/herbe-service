...
- ~~Confirm ERP service-module register codes~~ **largely resolved 2026-07-06** (owner export structures + halocron; `04-erp-sync.md` findings + `17-erp-register-reference.md`): `SVOVc`, `WSVc`, `SVOSerVc`, `DelAddrVc`, `ItemStatusVc`, `RLinkVc` verified; OK-flag stock semantics, record-links back-link, REST pricing limitation established. **Remaining for the demo-system probe**: charge-type row fields on `WSVc` rows, `RLinkVc` REST readability vs `getrecordlinks`-only, no-`updates_after` assumption on `SVOVc`/`WSVc`, service-contracts register code, `UserVc.Location` vs `ServLocation` convention, `ActVc` activity types + intake types per connection, WebExcellentAPI presence, `SVOVc`/`WSVc` PDF forms (ERP-side work in progress)
...
- **Worksheet is one-per-technician from day one** (round 6 correction: reverted from a shared lead+members worksheet — `02-data-model.md`, `WSVc.EMCode` is a single technician); Phase 1 UX plans one technician per job, so this ships without any crew UI. Crew jobs (several technicians, several worksheets sharing a `crewGroupId`) are usable data-model-wise from day one but get dedicated UX (grouped queues, crew booking chip) only with the Phase 2 dispatch board
...
- **Crew scheduling UX**: several technicians on one job — crew bookings with `crewGroupId`, moved as a group or detached individually; **one worksheet per technician, sharing the `crewGroupId`** (round 6, `02-data-model.md`), each logging own time/parts from own van, grouped in list/queue views so the crew job still reads as one job; one multi-person `ActVc` activity per crew job (`04-erp-sync.md`)
