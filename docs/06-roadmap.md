...
Status: v0.11 (2026-07-07 — **phases collapsed (owner): old Phase 1 and Phase 2 merged into one MVP build phase, because nearly all of old Phase 2 was config-UI-for / enrichment-of / scale-extension-of Phase-1 foundations — tied together, so pulled forward. Old Phase 3 → Phase 2, old Phase 4 → Phase 3. Phase 0 unchanged. The two later phases stay separate only because they stand cleanly on their own: contracts + recurring + customer experience (Phase 2), and optimization/intelligence (Phase 3).**). Previous: v0.10 (2026-07-07 — biometric unlock is a Phase 1 item via the WebAuthn platform authenticator in the installed PWA, alongside the PIN lock). Previous: v0.9 (2026-07-07, round 8: back office → Phase 1; crew grouped-by-job UI → Phase 1; charge-type enum closed; WebExcellentAPI service/contracts document support version-gated; portal integration reduced; `SVOVc` REST-create resolved; design-system import closed; provisioning ADR = Supabase branching). Previous: v0.8 (2026-07-06, round 7: `worksheetShadow` moved to Phase 1 minimal). Previous: v0.7 (round 6: worksheet model reverted to one-per-technician). Previous: v0.6 (live demo-ERP probe). Previous: v0.5 (owner round 4). Previous: v0.4 (round 3). Previous: v0.3 (spec-line merge). Sizing assumes a small team (2–3 devs + design shared with the suite); durations are calendar estimates, **re-planned after Phase 0** — and with the P1+P2 merge the combined build phase is large, so the estimate below is explicitly a Phase-0-exit re-plan item.
...
- ~~Confirm ERP service-module register codes~~ **resolved 2026-07-06** (owner export structures + halocron + a live demo-system probe; `04-erp-sync.md` + `17-erp-register-reference.md` + `19-demo-probe-results.md`): `SVOVc`, `WSVc`, `SVOSerVc`, `DelAddrVc`, `ItemStatusVc`, `RLinkVc`, `COVc` verified; OK-flag stock semantics, REST pricing limitation established; `RLinkVc` back-link → WebExcellentAPI `getrecordlinks`; no `updates_after` on `SVOVc`/`WSVc` (confirmed); `deletes_after` unreliable (confirmed); server-side filters work; `ActVc` types `SVP`/`SERV` on the demo tenant; WebExcellentAPI present, `SVOVc`/`WSVc` documents still missing. **Work Order (`WOVc`) chain — resolved 2026-07-07**: set `WSVc.WONr = -1` on create; no `WOVc` needed, two-step `SVOVc → WSVc` stands. **Charge-type enum — resolved 2026-07-07**: string set 31 (`0`=-, `1`=Invoiceable, `2`=Warranty, `3`=Contract, `4`=Goodwill — 1:1 with app charge types, distinct from `INVc.ItemType`); push writes the integer `1`–`4`, reads return the localized label (confirmed live on `SerNr 230022`); only `Invoiceable` charges, the other three are no-charge. (WebExcellentAPI version-gate source = the `systemversion` attribute on the REST `<data>` root, matched by an admin-set regex; `04-erp-sync.md`.)
...
- **`/api/ext/v1` service API** + hashed scoped bearer tokens (per company connection) + admin minting UI — re-cut to the reduced portal surface (the signoff/approval triggers; `08-suite-integration.md` §4)
...
- Notification/email sending via the portal's TemplateKey engine (reused) — worksheet PDF mail and rejection notices
...
Goal: proactive service and the customer-facing surface. (Was Phase 3 — kept separate because it stands cleanly on its own and builds on the Phase-1 document engine + `/api/ext` API.)
...
- **Customer surface = herbe.portal, exclusively** (owner 2026-07-05: herbe.service ships no customer-facing pages). Portal service modules: equipment + history (QR label deep-link target), request intake → service order, order status incl. "technician on the way" view, report PDFs, worksheet signoff, satisfaction feedback — built portal-side, reading `/api/ext/v1`. Tenants without portal: emailed report PDFs + on-site signature — nothing web-facing from service
...
Prioritize by pilot data, not upfront (was Phase 4):
...
Resolved this cycle and folded into the docs: crew model (one worksheet per technician), portal integration reduced to signoff/approval triggers + ERP middleman, design-system import, `WOVc` chain (`WONr = -1`), `SVOVc` REST-create (POST with no `SerNr`), charge-type enum + `Invoiced`-via-`IVVc`, order `Closed` = `DoneMark`, `COVc` inbound + `SVCVc` service levels, van-stock resolution, shadow-Kanban + `ActStateVc`, biometric-in-PWA.
