...
9. **`SVOVc` REST-create blocker, new 2026-07-07** (narrowed): outbound service-order creation has **never succeeded live** — the probe's `POST` returned HTTP 200 echoing the payload with no error field, yet nothing persisted (a silent "Jau reģistrēts" no-op; `19-demo-probe-results.md` §10). Likely a missing required field or create-mode flag (the `WSVc`/`WONr` case showed these registers enforce non-obvious requireds). Fix by replicating the ERP's own `SVOVc` creation field-set — as done for `WSVc` from `PasteSVOInWS` (owner to provide the `SVOVc` routine, or via halocron). `WSVc` create is unblocked; only the order-create step waits on this
