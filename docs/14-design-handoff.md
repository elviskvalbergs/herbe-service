...
4. **A `herbe.service` wordmark variant.** The inline-SVG logo component today has only `portal` (cyan dot) and `herbe` (red dot) variants; herbe.service needs its own — a dot colour and the `.service` text, calibrated to the same Poppins metrics as the sibling wordmarks (`07-ui-screens.md`). Brand decision, needed before the app shell ships.
