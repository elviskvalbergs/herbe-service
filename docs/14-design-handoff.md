...
| **Worksheet-confirmation revision-state badge** | `signed / superseded-by-correction / re-sign-requested / proceed-on-existing (audited)` — no vocabulary exists yet (`02-data-model.md` "Worksheet-confirmation lock vs. rejection/correction"). |
| **Print/PDF tokens** | Header/footer, table, and severity styling for generated documents (`12-documents-templates.md`): the built-in order-level service report renders from theme tokens with no DOCX template (the seeded default template). Verified 2026-07-07: no `@media print` styling exists anywhere in the design-system repo. |
...
- **P1 — Worksheet confirmation (signature capture)**: full-screen canvas, name field, legal text slot, content-lock confirmation, plus a **re-entry state** for "signature requested again" after a correction (see revision-chain pattern, §3).
...
- **P1 — ERP connection (A4) additions**: capability-probe result display, **activity-purpose map** (a six-row per-purpose type/symbol mapping table, `04-erp-sync.md`: `booking` / `worksheetShadow` (ships Phase 1, minimal) / `orderShadow` / `intake` / `workSegment` (Phase 1 config) / `historyImport`), invoice back-link field, poll cadence and maintenance-window settings, plus transformations editor (declarative maps + JS hooks), settings import/export, `/api/ext` token minting/revocation table.
...
6. Worksheet confirmation capture, incl. re-sign re-entry state (F5)
...
4. **A `herbe.service` wordmark variant.** The inline-SVG logo component today has only `portal` (cyan dot) and `herbe` (red dot) variants; herbe.service needs its own — the `.service` text plus its **squared dot in amber** (`--cat-amber` `#E08A2B`; owner decision 2026-07-08 — reads field / hi-vis, distinct from calendar-red and portal-cyan), calibrated to the same Poppins metrics as the sibling wordmarks (`07-ui-screens.md`). Add `--product-service: var(--cat-amber)` to `herbe-design-system/tokens.css` alongside the other `--product-*` accents (none exists yet), and the app's accent/dot reaches for it; the dot stays **square** (locked principle). Needed before the app shell ships.
