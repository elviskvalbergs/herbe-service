...
- **P1 — Worksheet execution screen family**: big-target quantity stepper, part row with stock-location badge + `addedBy` crew attribution, per-row **charge-type selector** (4 values, field-policy gated), per-row **service-item attribution picker** (when a worksheet spans multiple nodes), start/stop **timer control** (work/travel/waiting, with a **return-travel variant that stays live after the worksheet is `Done`** — a timer that must remain tappable in an otherwise "closed" document, no existing pattern for this), pause-with-reason picker, status-transition button with blocking-requirements list (field policies: "missing: km, remedy code").
