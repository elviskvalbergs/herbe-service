...
- **Signoff — on-site baseline, portal for everything remote** (owner 2026-07-05: all remote customer interaction lives in the portal): the baseline every tenant has is the on-site **canvas signature** for the doorstep case. Remote customer **signoff** happens in **herbe.portal** — the worksheet-report signoff module (`08-suite-integration.md` §4) for reports; documents needing a *qualified* signature (Smart-ID / Mobile-ID — quote confirmations, compliance certificates) route through the portal's **signing descriptor — the primary mechanism** (the customer signoff family POR-2/6/7, `13-suite-change-requests.md`; `06-roadmap.md` Phase 2). The **`ActVc` activity-as-vessel path is the no-module fallback**, for tenants whose portal lacks the signing module (`04-erp-sync.md` mapping): the generated document attaches to an activity via record links, the portal presents it like a delivery confirmation, and the outcome (approved / signed / rejected + comment) travels back as the activity's workflow stage + text — landing in herbe.service as a document status change. The fallback works at **tier 0** (through the ERP, no suite-to-suite API), which is its value — but it is the fallback, not the design. Any signature locks the worksheet content the document renders from; a signed PDF becomes a new immutable version.
...
- **Phase 1**: built-in worksheet report PDF (tenant-themed) **and** the DOCX template engine — placeholders, loops, conditionals, images; computed fields & display rules (shared with built-in documents); template library + test render; selection rules; document number series; automatic generation on approval; order confirmation as a DOCX template type (`06-roadmap.md` is authoritative). Rendering-pipeline ADR (converter choice).
