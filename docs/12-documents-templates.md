...
Status: v0.2 (2026-07-07). Changes: signing precedence aligned with `06-roadmap.md`/POR-2 (portal signing descriptor primary, `ActVc` activity-vessel is the no-module fallback); built-in order-confirmation PDF dropped from Phase 1 (owner decision — Phase 1 is the worksheet report only; order confirmation stays a Phase-2 DOCX template type); manual-generation screen entry points referenced (`07-ui-screens.md` F4/O2). Previous: draft v0.1 (2026-07-03). Service companies must produce customer- and authority-facing documents that are far richer than a generic "work report": inspection certificates, compliance protocols, per-system test reports, order confirmations — often with a legally prescribed layout and per-service-item/group content. The customer defines the document; the app fills it.
