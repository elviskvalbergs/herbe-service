...
- **Sign-off — on-site baseline, portal for everything remote** (owner 2026-07-05: all remote customer interaction lives in the portal): the baseline every tenant has is the on-site **canvas signature** for the doorstep case. Remote customer **sign-off** happens in **herbe.portal** — the worksheet-report sign-off module (`08-suite-integration.md` §4) for reports; documents needing a *qualified* signature (Smart-ID / Mobile-ID — quote confirmations, compliance certificates) route through the portal's **signing descriptor — the primary mechanism** (the customer sign-off family POR-2/6/7, `13-suite-change-requests.md`; `06-roadmap.md` Phase 3). The **`ActVc` activity-as-vessel path is the no-module fallback**, for tenants whose portal lacks the signing module (`04-erp-sync.md` mapping): the generated document attaches to an activity via record links, the portal presents it like a delivery confirmation, and the outcome (approved / signed / rejected + comment) travels back as the activity's workflow stage + text — landing in herbe.service as a document status change. The fallback works at **tier 0** (through the ERP, no suite-to-suite API), which is its value — but it is the fallback, not the design. Any signature locks the worksheet content the document renders from; a signed PDF becomes a new immutable version.
...
- **Phase 3**: compliance certificates per node/coverage with subtree annexes; contract-cycle documents; **customer sign-off in herbe.portal — signing descriptor primary, `ActVc` activity-vessel as the no-module fallback** (POR-2/6/7; tier-0 fallback path); template set in settings export/import.
