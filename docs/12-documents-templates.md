...
- **Phase 1**: built-in **order-level** service report PDF (tenant-themed; renders all the order's approved worksheets, the document presented for signoff) **and** the DOCX template engine — placeholders, loops, conditionals, images; computed fields & display rules (shared with built-in documents); template library + test render; selection rules; document number series; automatic generation on approval; order confirmation as a DOCX template type (`06-roadmap.md` is authoritative). Rendering-pipeline ADR (converter choice).
