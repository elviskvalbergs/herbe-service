...
Status: v0.4 (2026-07-07 — roadmap phases collapsed: old Phase 1+2 merged into Phase 1, old Phase 3→Phase 2, old Phase 4→Phase 3; the DOCX template engine and computed fields are now Phase 1, compliance/contract/signoff docs now Phase 2, and the Phase-1-report/Phase-2-engine split is collapsed into a single Phase 1). Previous: v0.3 (2026-07-07 — customer **signoff** adopted as the umbrella term for the portal approval/signature flow, POR-2/6/7). Previous: v0.2 (2026-07-07). The portal signing descriptor is the primary signoff mechanism; the `ActVc` activity-vessel is the no-module fallback. Phase 1 ships both the built-in worksheet report and the DOCX template engine — order confirmation is a Phase-1 DOCX template type. Manual "Generate document…" entry points on `07-ui-screens.md` F4/O2. Previous: draft v0.1 (2026-07-03). Service companies must produce customer- and authority-facing documents that are far richer than a generic "work report": inspection certificates, compliance protocols, per-system test reports, order confirmations — often with a legally prescribed layout and per-service-item/group content. The customer defines the document; the app fills it.
