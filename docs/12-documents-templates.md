...
- **Phase 3**: compliance certificates per node/coverage with subtree annexes; contract-cycle documents; **customer signoff in herbe.portal — signing descriptor primary, `ActVc` activity-vessel as the no-module fallback** (POR-2/6/7; tier-0 fallback path); template set in settings export/import.
