...
Status: v0.4 (2026-07-07). Word-based mail-merge templating for customer- and authority-facing documents far richer than a generic "work report" (inspection certificates, compliance protocols, per-system test reports, order confirmations, often with a legally prescribed layout and per-service-item/group content): the customer defines the document, the app fills it. Phase 1 ships both the built-in worksheet report and the DOCX template engine with computed fields; order confirmation is a Phase-1 DOCX template type; compliance/contract/signoff docs are Phase 2. The portal signing descriptor is the primary signoff mechanism (customer "signoff" is the umbrella term for the portal approval/signature flow, POR-2/6/7); the `ActVc` activity-vessel is the no-module fallback. Manual "Generate document…" entry points on `07-ui-screens.md` F4/O2.
...
|---|---|---|
| Order service report / protocol | ServiceOrder | **all the order's approved worksheets rendered as sections** (per technician / per visit) + customer, site, service item node(s) + paths + models, rows, time, checklist results (values, bounds, pass/fail), coverage incl. exception lists, media, the order-level signoff signature. This is the customer-facing report presented for signoff; a solo job is just the one-worksheet instance, a crew job merges the siblings by `crewGroupId`. A per-worksheet render stays available as an internal/manager artifact (`02-data-model.md`). |
| Service order document (confirmation, quote — Phase 1, with the template engine) | ServiceOrder | rows, requested/promised dates, contract terms reference, price summary (role-gated), booking slots + technicians |
...
| Contract summary (Phase 2) | Contract | covered nodes tree, PM schedule, response terms |
...
- **Triggers**: automatic on worksheet approval (per document type config), manual ("Generate document…" on worksheet/order/node with template override — screen entry points on the worksheet execution and order detail screens, `07-ui-screens.md` F4/O2), and Phase 2 contract-cycle documents (e.g. yearly certificate after the annual inspection completes).
- **Pipeline**: background job (the `03-architecture.md` scheduled/queued pattern — rendering never blocks the approval transition): build context → DOCX merge → PDF conversion → store. PDF conversion needs a real converter (LibreOffice-based service, e.g. Gotenberg, or an equivalent API) — sized/chunked to the Vercel function limits like other jobs; implementation choice is a Phase 1 ADR.
...
- **Signoff — two scopes, portal-set method** (`02-data-model.md`): scope is `require_worksheet_confirmation` (on-device, per-worksheet — the doorstep case, no customer-facing interface, so it's the **portal-less baseline**) and/or `require_order_signoff` (order-level, customer-facing); the method `confirm | sign | digital_sign` is a herbe.portal setting and only the portal path reaches qualified `digital_sign`. Remote customer **order-level signoff** happens in **herbe.portal** — the order-report signoff module (`08-suite-integration.md` §4): once the order's worksheets are all approved, the portal requests the combined order report and presents it. Documents needing a *qualified* signature (Smart-ID / Mobile-ID — quote confirmations, compliance certificates) route through the portal's **signing descriptor — the primary mechanism** (the customer signoff family POR-2/6/7, `13-suite-change-requests.md`; `06-roadmap.md` Phase 2). The **no-module fallback**, for tenants whose portal lacks the signing module (`04-erp-sync.md` mapping): the generated document attaches to the **`orderShadow`** activity via record links (no dedicated vessel purpose — the shadow carries it like any attachment), the portal presents it like a delivery confirmation, and the outcome (`confirmed` / `rejected` + reason) travels back as that activity's workflow stage + text — landing in herbe.service as an order signoff status change. The fallback works at **tier 0** (through the ERP, no suite-to-suite API), which is its value — but it is the fallback, not the design. **Signoff is resendable**: a `rejected` decision re-opens the order (add a booking + worksheet, regenerate and re-present); a low satisfaction rating is a separate axis and never blocks closure. Any signature locks the order's worksheet content the report renders from; a signed PDF becomes a new immutable version.
...
All app-owned; nothing crosses the ERP API except the finished document as an attachment/link. (ERP-generated documents — invoices — are the ERP's own; where WebExcellentAPI is present the app can *fetch* those PDFs for display, which is the existing capability flag, not this engine.) The WebExcellentAPI-dependent steps here for **service and contracts** documents — attaching the generated document to the `orderShadow` activity, creating/reading the attached activity, fetching the resulting PDFs — follow the per-connection **service/contracts version gate** (`04-erp-sync.md` API tiers): enabled when the installation's `systemversion` matches the configured pattern, hidden otherwise; the emailed-PDF + on-site canvas-signature fallback is unaffected. (Fetching *invoice* PDFs already works on current versions — presence-gated only.)

- **Phase 1**: the **default built-in order-level service report** — a **seeded default template**, tenant-themed, that renders all the order's approved worksheets into one PDF with no authoring required (the document presented for signoff), so every tenant has a working report out of the box **and** portal-less tenants can email it — **and** the DOCX template engine for tenants that want to override it: placeholders, loops, conditionals, images; computed fields & display rules (shared with built-in documents); template library + test render; selection rules; document number series; automatic generation on approval; order confirmation as a DOCX template type (`06-roadmap.md` is authoritative). Rendering-pipeline ADR (converter choice). The portal-facing `GET /api/ext/v1/orders/{id}/report` endpoint that serves it for signoff is Phase 2 (`08-suite-integration.md` §4), but the report + default template themselves are Phase 1.
- **Phase 2**: compliance certificates per node/coverage with subtree annexes; contract-cycle documents; **customer signoff in herbe.portal — signing descriptor primary, the `orderShadow`-attached document as the no-module fallback** (POR-2/6/7; tier-0 fallback path); template set in settings export/import.
- **Phase 3**: AI-assisted drafting of work descriptions feeding the documents (already planned) — templates unchanged, better inputs.
