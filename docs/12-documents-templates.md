...
- **Signoff — two scopes, portal-set method** (`02-data-model.md`): scope is `require_worksheet_signature` (on-device, per-worksheet — the doorstep case, no customer-facing interface, so it's the **portal-less baseline**) and/or `require_order_signoff` (order-level, customer-facing); the method `confirm | sign | digital_sign` is a herbe.portal setting and only the portal path reaches qualified `digital_sign`. Remote customer **order-level signoff** happens in **herbe.portal** — the order-report signoff module (`08-suite-integration.md` §4): once the order's worksheets are all approved, the portal requests the combined order report and presents it. Documents needing a *qualified* signature (Smart-ID / Mobile-ID — quote confirmations, compliance certificates) route through the portal's **signing descriptor — the primary mechanism** (the customer signoff family POR-2/6/7, `13-suite-change-requests.md`; `06-roadmap.md` Phase 2). The **`ActVc` activity-as-vessel path is the no-module fallback**, for tenants whose portal lacks the signing module (`04-erp-sync.md` mapping): the generated document attaches to an activity via record links, the portal presents it like a delivery confirmation, and the outcome (`confirmed` / `rejected` + reason) travels back as the activity's workflow stage + text — landing in herbe.service as an order confirmation status change. The fallback works at **tier 0** (through the ERP, no suite-to-suite API), which is its value — but it is the fallback, not the design. **Signoff is resendable**: a `rejected` decision re-opens the order (add a booking + worksheet, regenerate and re-present); a low satisfaction rating is a separate axis and never blocks closure. Any signature locks the order's worksheet content the report renders from; a signed PDF becomes a new immutable version.
...
- **Phase 2**: compliance certificates per node/coverage with subtree annexes; contract-cycle documents; **customer signoff in herbe.portal — signing descriptor primary, `ActVc` activity-vessel as the no-module fallback** (POR-2/6/7; tier-0 fallback path); template set in settings export/import.
