...
| C2 | Major | Booking had no status model at all (the competitive analysis itself calls independent booking statuses a key D365/SFS lesson). | ✅ `02`: `planned → confirmed → cancelled` (+rescheduled as cancel-and-recreate), aligned with calendar's booking vocabulary; execution progress stays on Worksheet. |
| C3 | Major | ServiceOrder status flow (`Planned…Confirmed`) had no defined relationship to worksheet statuses — who flips what was ambiguous. | ✅ `02`: explicit derivation rules; only accept and close/cancel are manual. |
