...
So the projection = **rules + overrides + blackouts**; the engine folds them in. This override layer is **net-new** — the reused repeat engine clones-then-edits (pause and `after_n` live in its schema/cron layer, and it has no per-instance override at all — §5), so a deliberate service-overlay addition keeps the schedule freely editable while still projection-first. When the near horizon materialises, overrides/blackouts are already baked in, so committed bookings never contradict the plan.
...
- **Rich patterns** — daily; weekly incl. multi-weekday; monthly absolute *and* nth-weekday; yearly absolute *and* relative; interval count; end conditions and pause. **Copy-scope caveat (verified 2026-07-15):** the *pure engine file* enforces only `never`/`on_date`; **`after_n` and `paused` live in the calendar's schema + cron layer** (`materialized_count`/`max_occurrences` columns, cron skip — migrations 58/59/68), not in `repeatRules.ts`. Copy the **pattern** (engine + those columns + the cron enforcement), not just the file, or both silently disappear.
...
| `blackoutCalendarId` | per-customer closed windows; a **tenant-default holiday calendar** blacks out *unassigned* projected occurrences, refined to the technician's **per-person national holidays** (herbe.calendar's import, CAL-10, `13`) at materialization once assigned (owner 2026-07-15) — not a bespoke list |
