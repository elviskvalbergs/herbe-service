...
| `ItemType` ✓ | M4Int, on the `WSVc` row itself (`WSIVVc` carries its own copy — see below; `SVOVc` rows carry it too) | **Closed 2026-07-07 (owner-provided enum, string set 31): the charge-type discriminator.** `SetBegin(31)`: `0` = "-", `1` = Invoiceable, `2` = Warranty, `3` = Contract, `4` = Goodwill — **1:1 with the app's charge types**. Editable in the ERP UI pre-OK (`WSDClassItemTypeEFAfter` → `WSVc_PasteItemType` recalculates the row on change); the ERP computes a context-derived **default** — the live probe read back Latvian `"Jāizr.rēķ."` (= Invoiceable) on a plain row, which is what the 2026-07-06 probe observed as "ERP-computed". REST reads return the **localized label string**, not the integer; the write format (integer vs label) is the one remaining Phase-0 check, with `QtyInvbl` observed on the same write test. Print labels: `USetStr` 7761/7762/7763/7768 (`PrintWSRows`, `WordWSForm.hal`). **Name collision**: `INVc.ItemType` (item classification Plain/Stocked/Structured/Service; the `kItemType*` constants) is a different field — do not conflate. Charge type is **app-owned** in herbe.service; the push maps it 1:1 to this enum once the write format is verified. Warranty/contract *context* lives on the service-order item (`SVOSerVc`) |
