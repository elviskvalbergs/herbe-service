...
Status: v0.4 (2026-07-08). The developer reference behind the mapping table in `04-erp-sync.md`; for registers not listed here (CUVc, INVc, IVVc, ActVc, …) the portal/calendar codebases and halocron are the reference. Cross-checked against the halocron register dictionary: `SVOVc` at 111 fields, `WSVc` header/row split annotated with row fields marked HAL-source-verified vs unconfirmed, row `ItemType` documented as the charge-type enum (string set 31, pushed as integer `1`–`4`); `ActStateVc` (activity workflow states behind the shadow-Kanban) and `SVCVc` (service levels / recurring-service schedules behind the `COVc` row `SVCCode`) added. Sources: owner-provided Standard ERP export structures (`SVOVc`, `WSVc`, printed 2026-07-06 from a live system), the halocron register dictionary (`list_registers`), and a live demo-system probe (`19-demo-probe-results.md`) that corrected two field-behavior assumptions (`RLinkVc` record-id format, `WSVc.WONr` requiredness) and added the `COVc` and `WSIVVc` sections below.
...
| `StandProblem`, `StdProblemMod` | M4Code 20 | standard problem codes (→ work templates, Phase 2) |
...
`Code` (M4Code 10 — the technician person code used in `WSVc.EMCode` and `ActVc` persons; **owner-confirmed as our identity-link target**), `Name`, `emailAddr` / `LoginEmailAddr` (identity matching), `Location` (M4UStr 10 — **default stock location = van stock**, owner), `ServLocation` (M4UStr 10 — service-module variant; confirm tenant convention), `ReservLocation`/`ReservLocAccess`, `JobGroup`, `Department`, `SalesGroup`, `CostPrHour`/`PricePrHour` (labor cost/price — Phase 1 payroll-side reporting candidate), `Closed`, `TerminatedFlag`. Has `UUID`/`ServerSequence` → base register.

**Live-probe finding (2026-07-06, `19-demo-probe-results.md` §7)**: on the demo system, neither `Location` nor `ServLocation` is populated on any of the 40 users, including the two technicians actually referenced by real `WSVc.EMCode` values. `UserVc.Code` matching `WSVc.EMCode` exactly was confirmed for both. The van-stock convention therefore isn't assumable from data. Resolved by the resolution chain in `04-erp-sync.md`: `WSVc.Location` = `UserVc.Location` → the connection's **"main service location"** setting → ERP main stock (first non-blank), so an unpopulated `Location`/`ServLocation` never blocks a push. A tenant that keeps service stock on `ServLocation` just points the setting there.
