...
Last updated: **2026-07-17** (preview @ PR #9 sync-runner + PR #10 phase-2-recurrence; WS1 status corrected from "Done" to "substrate only" — verified against the repo; WS2 core + password/TOTP for admin merged via PR #11 (`feature/service-phase1-ws2-roles-identity`); WS4 ERP outbound landed on `feature/service-phase1-erp-outbound`, PR #13 into preview — includes adopting WS2's `identity_links` as the technician EMCode convention, see §2b/§5).
...
2. **WS2 remainder — WebAuthn, Baltic eID, Entra ID OIDC, seat/license enforcement** — core (roles/capabilities, session revocation, device registry, ERP identity link, admin password+TOTP) is done, see §5. Touches `lib/auth` only. No overlap with WS3/WS12. Its login/enrolment UI needs WS1's shells first (see §5's frontend note) — coordinate with WS1/WS9 or build against minimal pages.
...
**All of WS2 is backend/API only — there is NO auth frontend yet.** herbe-service's `app/` is still essentially the create-next-app skeleton (stock `page.tsx`, Geist fonts in `layout.tsx`, one unstyled customers `<ul>`); there is no `/login` page at all (even though `authConfig` sets `pages.signIn = '/login'`), so *neither* magic-link *nor* password+TOTP login is reachable from a browser, and there is no settings surface for password/TOTP enrollment or device management. Building any of it needs WS1's foundation first (the design-system bootstrap — Poppins, `--herbe-*` tokens, the amber-dot `.service` inline-SVG wordmark — plus the office shell), which is not done. So the auth UI is WS1/WS9 frontend work, not a WS2 loose end: a `/login` page (both providers), a `/settings/security` page (set password, QR-render the `otpauthUri`, show recovery codes, disable — needs a `qrcode` dep, deliberately not added yet), and device-registry management screens.
