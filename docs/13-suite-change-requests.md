...
| POR-2 | portal | Delivery-confirmation (signoff) flow accepts our documents (activity vessel) | Phase 2 | config/small feature | on-site canvas signature + emailed report PDF |
...
| POR-4 | portal | Worksheet-approval trigger (`POST /api/ext/v1/worksheets/{id}/confirm`); the wider service-data feed is the portal team's own scope | service Phase 1 (API) | reduced/committed | non-portal tenants: emailed report PDF + on-site canvas signature |
...
| POR-7 | portal | Token-authenticated "send worksheet/document for signoff" API (customer signoff) | Phase 2 (signing flow) | small feature | tier-0 poll-based flow already works end-to-end — this is a latency optimization only, not a blocker |
...
3. **Satisfaction feedback write-back (worksheet-level)** — the worksheet is what the customer signs off, so feedback rides it: an optional `feedback` block on `POST /api/ext/v1/worksheets/{id}/confirm` (rated with the signature) plus a standalone `POST /api/ext/v1/worksheets/{id}/feedback` for a later survey link (both idempotent; owner 2026-07-08 — **direct API, not ERP-mediated**). Rolls up to order/customer/period for reporting. Ships with the Phase 2 customer-experience work, but the mechanism is committed now, not "future".
