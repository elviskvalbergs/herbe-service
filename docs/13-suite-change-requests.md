...
| CAL-3 | calendar | Documented API-token endpoints + activity webhook or delta API | Phase 1–2 (tier 1) | new feature | tier 0 polling, ICS feeds |
| CAL-4 | calendar | Smart Booking: service-intake template fields (asset reference) + reschedule/cancel links in the confirmation email | Phase 2 | small feature | free-text serial field + QR-prefilled booking links |
| CAL-5 | calendar | Availability query API (merged busy-times) | Phase 3 | new feature | herbe.service checks only its own bookings |
| CAL-6 | calendar | Service-activity recognition (badge/colour for configured service activity types) | Phase 1–2 | small feature | service activities render as ordinary activities |
| CAL-7 | calendar | Service context + "Open in herbe.service" deep link in `ActivityDrawer`/`ActivityBlock` | Phase 1–2 | small feature | context lives in the activity text fields service writes anyway |
| CAL-8 | calendar | Guarded editing of service-locked activities | Phase 2 | small feature | service bounces illegal inbound moves to the dispatcher inbox (tier-0 conflict rule) |
| CAL-9 | calendar | Embeddable team/dispatch calendar view (scoped to a service company's technicians + service bookings) for in-app reassignment | Phase 2 | new feature | deep-link out to herbe.calendar, or use the herbe.service dispatch board (`22`/P1 WS10) for reassignment |
| CAL-10 | calendar | Expose per-person **national-holiday** data (calendar already imports it per person/technician) via a scoped read | Phase 2–3 | small feature | herbe.service maintains its own holiday calendar config, or honours weekends only |
| POR-1 | portal | Invoice ↔ service order cross-link in portal UI | Phase 2 | small feature | customer matches by invoice text reference |
| POR-2 | portal | **Order signoff** — the report/confirm API (`GET /orders/{id}/report` + `POST /orders/{id}/confirm`, resendable) plus the ERP-mediated delivery-confirmation fallback (order report + compliance docs on the `orderShadow` activity) | Phase 2 | config/small feature | on-site canvas signature + emailed report PDF |
| POR-3 | portal | Cross-links from portal to service customer pages — no longer applicable (no service customer pages exist) | — | superseded | portal is the only customer surface |
| POR-4 | portal | Reduced native service-data feed — near-term realization is the order-signoff API (POR-2); the wider module (equipment, intake, tracking, feedback) is the portal team's own scope | service Phase 2 (API framework Phase 1) | reduced/committed | non-portal tenants: emailed report PDF + on-site canvas signature |
| POR-5 | portal | Messaging threads on service entities | optional, later | feature | service questions via our notifications + email |
| POR-6 | portal | Token-authenticated "send quotation for signoff" API | Phase 2 (quote flow) | small feature | human sends from the portal quotation view |
| POR-7 | portal | Token-authenticated "send order report / document for signoff" API (customer signoff) | Phase 2 (signing flow) | small feature | tier-0 poll-based flow already works end-to-end — this is a latency optimization only, not a blocker |
...
| SUITE-2 | both | Repo access — `BITBUCKET_APP_PASSWORD` via the Bitbucket REST API | — | done | mirrors optional convenience |
| SUITE-3 | both | Shared theme-token vocabulary — design-system repo is canonical (`tokens.css` + `handovers/SERVICE.md`) | — | done | consume the canonical repo |
...
### CAL-3 — Documented API endpoints + event push (tier 1, Phase 1–2)
...
### CAL-4 — Smart Booking service-intake fields (Phase 2)
...
### CAL-5 — Availability API (Phase 3)
...
### CAL-6 — Service-activity recognition (Phase 1–2, small; `08` §3 C1)
...
### CAL-7 — Service context + deep link on the activity (Phase 1–2, small; `08` §3 C2)
...
### CAL-8 — Guarded editing of service-locked activities (Phase 2, small; `08` §3 C3)
...
### CAL-9 — Embeddable team calendar view for reassignment (Phase 2; `23` §4.4)

The main pain in managing recurring service at scale is reassignment: technicians get sick or reallocated, and keeping a **team view** while moving work is hard (owner 2026-07-15). herbe.service generates the work as `ActVc`/bookings anyway, so rather than rebuild a team calendar, ask calendar for an **embeddable team/dispatch view** — scoped to one service company's technicians and its service bookings — that herbe.service hosts in-app, with drag-to-reassign across technicians. This keeps the "no second Kanban / no second calendar" rule (`22` §2): the calendar owns time, the service app embeds it. Fallback: a deep link out to herbe.calendar, or the herbe.service dispatch board (`22`/P1 WS10) does reassignment without the full team-calendar UX. **Owner (2026-07-15): "add to the calendar required changes whatever you need" — so this is a committed ask; the exact embedding mechanism (iframe/scoped route vs. shared component) is for the calendar team to choose.**

### CAL-10 — Per-person national-holiday data (Phase 2–3, small; `23` §3)

herbe.calendar already **imports national holidays per person/technician**. The recurring-service scheduler wants holidays as blackout dates when it projects/materializes due services (skip/shift a service that falls on a technician's public holiday — beyond the SVCVc weekend rule). Ask: expose that per-person holiday data via a scoped read (naturally alongside the CAL-5 availability API, or a small dedicated endpoint). Fallback: herbe.service maintains its own per-tenant holiday calendar config (more setup, drift risk), or honours weekends only. Piggybacking on calendar's existing import is the owner's preference (2026-07-15).

## herbe.portal

The portal hosts the customer surface (exclusivity confirmed 2026-07-05); herbe.service ships no customer pages. The portal's service module is the portal team's **independent** product — it must stand on its own, with limited functionality, even without herbe.service; their v1 design note lives at `herbe-portal/docs/superpowers/specs/2026-07-04-service-modules-design.md`. The **only** direct service↔portal coupling in the near term is the customer **signoff** API calls (POR-2/6/7) — the order-report signoff (POR-2), the quotation signoff trigger (POR-6) and the document signoff trigger (POR-7) — plus, tentatively, label/QR handling. **The ERP is the connecting piece for all actual data: the portal reads service data from the ERP, never from herbe.service.** Portal reading data directly from herbe.service is a **future** step, taken once the module's shape settles (`08-suite-integration.md` §4). POR-1/POR-2 remain small touchpoints; POR-3 is superseded.

### POR-1 — Invoice ↔ service order cross-link (Phase 2, small)
...
### POR-2 — Delivery-confirmation (signoff) flow for service documents (Phase 2, mostly config)

Portal already routes documents to customers for approval and Smart-ID/Mobile-ID signature (delivery confirmations). `12-documents-templates.md` reuses exactly that flow for *qualified* signatures (customer **signoff**), with the `orderShadow` activity as the carrier: our generated document attaches to it via record links; the portal presents it like a delivery confirmation; the outcome returns as that activity's state (`ActState`). Ask: confirm this generalization — specifically that (a) the portal's flow can be configured to pick up our activity types, and (b) the outcome writes a state + comment we can poll. If their flow is invoice/delivery-hardcoded, this becomes a small feature request. Fallback: the on-site canvas signature plus the emailed report PDF (the baseline for tenants without portal anyway — herbe.service hosts no customer-facing approval page).

### POR-3 — Cross-links to service customer pages (superseded)
...
### POR-4 — Portal service module (reduced)

The portal service module is developed independently by the portal team and must stand on its own, with limited functionality, even without herbe.service — what exactly it becomes is still moving. Near-term direct integration is only:

1. **Order signoff trigger (resendable)** — signoff is on the order, not the individual worksheet: once all the order's worksheets are approved, the portal requests the combined order report (`GET /api/ext/v1/orders/{id}/report`) and posts the customer decision to `POST /api/ext/v1/orders/{id}/confirm` (`confirmed | rejected` + optional `reason`), scoped bearer auth. A `rejected` decision keeps the order open — the office adds a booking + worksheet and re-requests signoff. Confirmation is independent of the worksheet-level ERP push (parts/time are accounted on manager approval regardless).
2. **Quotation signoff trigger** — POR-6; the decision flows back through `QTVc`.
3. **Satisfaction feedback write-back (order-level)** — a separate axis from confirmation: an optional `feedback` block on `POST /api/ext/v1/orders/{id}/confirm` (rated with the signoff) plus a standalone `POST /api/ext/v1/orders/{id}/feedback` for a later survey link (both idempotent; owner 2026-07-08 — **direct API, not ERP-mediated**). Feeds per-technician/customer/period reporting; a low rating never blocks closure. Ships with the Phase 2 customer-experience work, but the mechanism is committed now, not "future".
4. **Tentatively, label/QR handling** — the portal side of the resolver in `08-suite-integration.md` §4a.

The rest of the wider read API (service items + history, orders/ETA, request intake) is **future** — re-cut against the portal module's settled shape; everything else flows through the ERP as middleman. Fallback / non-portal tenants: the emailed report PDF and the on-site canvas signature — herbe.service exposes nothing web-facing to customers.

### POR-6 — API to trigger quotation signoff (Phase 2, small)
...
### POR-7 — API to trigger worksheet/document signoff (Phase 2, small)
