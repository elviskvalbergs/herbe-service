...
| POR-4 | portal | Order signoff trigger (`GET /api/ext/v1/orders/{id}/report` + `POST /api/ext/v1/orders/{id}/confirm`, resendable); the wider service-data feed is the portal team's own scope | service Phase 1 (API) | reduced/committed | non-portal tenants: emailed report PDF + on-site canvas signature |
