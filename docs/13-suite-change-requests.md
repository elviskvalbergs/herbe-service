...
3. **Satisfaction feedback write-back** — the portal calls `POST /api/ext/v1/orders/{id}/feedback` (idempotent) with the customer's rating + comment (owner 2026-07-08 — **direct API, not ERP-mediated**), so service reporting has per-technician/period satisfaction. Ships with the Phase 2 customer-experience work, but the mechanism is committed now, not "future".
