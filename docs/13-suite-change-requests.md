...
| POR-2 | portal | Delivery-confirmation (signoff) flow accepts our order report + compliance docs (attached to the `orderShadow` activity) | Phase 2 | config/small feature | on-site canvas signature + emailed report PDF |
...
| POR-4 | portal | Order signoff trigger (`GET /api/ext/v1/orders/{id}/report` + `POST /api/ext/v1/orders/{id}/confirm`, resendable); the wider service-data feed is the portal team's own scope | service Phase 2 (endpoints; API framework Phase 1) | reduced/committed | non-portal tenants: emailed report PDF + on-site canvas signature |
