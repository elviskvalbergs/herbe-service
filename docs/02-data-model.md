...
Status flow: `New → Accepted → Planned → In progress → Work done → Confirmed → Invoiced → Closed` (+ `Cancelled`). "Invoiced" is set by ERP sync-back, never in the app. **"Closed" is likewise ERP-sync-set, not a manual or derived app transition** (owner, 2026-07-06 — round 6): `SVOVc` has no field named exactly `Closed` in the verified export (`17-erp-register-reference.md`), so the source field (candidates: `DoneMark`/`InvMark`, or a field outside the 82 mapped so far) is a **Phase 0 demo-probe confirmation item** (`18-demo-probe-handoff.md`); until confirmed, the app treats an order as terminal at `Invoiced` and shows `Closed` only once the poll observes the ERP's own completion signal.
