...
- **Scheduled jobs**: `GET /api/cron/*` routes with `Bearer ${CRON_SECRET}` and Postgres cron-locks — a **table-based lock** (the calendar's pattern), **not** session-scoped advisory locks: Supabase's connection pooler doesn't reliably hold advisory locks across pooled connections, exactly the reason the calendar moved off them on Neon (`08-suite-integration.md` §6; the two siblings differ here — portal uses advisory locks on direct connections, calendar uses the table-based lock). **Decision (product owner, 2026-07-05): Vercel cron is the primary scheduler for now**; `scripts/herbe-service-cron.sh` + `CRON-HANDOFF.md` are written and maintained from day one as the documented fallback, switched to only if Vercel cron's limits bite. To live within those limits (1-minute floor, one schedule per route), per-register/per-connection cadences are **not** separate cron entries: one frequent dispatcher route (`/api/cron/sync-tick`, every minute) reads due-times from the DB and fans out internally to whatever registers/connections are due — cadence stays tenant configuration, the cron surface stays tiny.
...
- **Sentry** (`@sentry/nextjs` with tunnel route) — suite convention; **pino** logging — portal convention, adopted here (the calendar runs no structured logger).
...
**Scoped replication (owner-approved classical design, 2026-07-06).** Field devices don't replicate the whole tenant — the delta feed is **per-user filtered**, and a raw high-water mark can't express "this record left your scope" (a job reassigned away produces no row update the old device would see, yet its data — including gate codes — must leave that phone). Classical publish/subscribe partitioning solves it:
- **Scope definition** per user: (a) *assignment-scoped* entities — orders, worksheets, bookings and their dependents (media, checklist results, site access instructions) where the user is lead/member/booked within the briefcase horizon (N days back and forward, tenant setting) plus their open history; (b) *reference closure* — the customers, sites, service items (+ their history) those documents point at; (c) *tenant-wide small registers* replicated unfiltered — item catalog + compatibility, checklist templates, and **stock levels per an `offline_stock_scope` setting** — default **all locations** (the parts-alternatives lookup, `11-service-items-and-parts.md`, needs cross-location stock in the briefcase; F8 cross-location search, `07-ui-screens.md`), reducible to the user's **own van location only** where the replication load proves too high. Office roles are tenant-wide by role, not scope-listed.
- **Scope membership table**: server-side `(userId, entityType, entityId, inScopeSince, outScopeSeq)` rows, maintained transactionally by the same mutations that change assignments/bookings and by a nightly horizon-advance job. Membership rows carry their **own `changeSeq`**, so the per-user delta feed is a join: changed rows currently in scope, plus **scope-exit records** (`outScopeSeq` set) which the client applies as local purges — indistinguishable from tombstones in client mechanics, but the server keeps the data (it was a subscription change, not a delete).
- **Scope-entry backfill**: when a record enters scope (new assignment), the feed emits it as a full upsert regardless of its last `changeSeq` — the membership row's seq drives inclusion, not the record's.
- **Outbox interaction**: ops referencing a record that has since left scope are still accepted server-side (physical facts don't expire with reassignment); the device just no longer receives updates for it after the purge.
- Enforcement is server-side in the feed query; the client never filters for security. This is the same briefcase model Salesforce ships; the membership table is the standard implementation on Postgres.
...
- Worksheet facts (rows, time, media, checklist values): technician wins — these are statements of physical fact. Conflicts are rare by construction: a worksheet has exactly one technician-author (crew jobs are N worksheets sharing a `crewGroupId`, `02-data-model.md` — no shared document, no per-row `addedBy`), so same-fact contention means office vs. field, never technician vs. technician.
...
- **Background jobs**: Vercel cron (primary; ops-runner as documented fallback — scheduling decision above) hitting `GET /api/cron/*` with table-based cron-locks (Supabase-pooler-safe, `08-suite-integration.md` §6), sized to finish within function duration limits — chunk large companies/registers across runs (calendar's `syncAllErp` batching: `Promise.allSettled` per connection, 60 s per-connection timeout, 500-row upsert batches, page limit ceilings). Job inventory: per-register ERP polls, ERP push queue drain, key-sweep reconciliation, **HistoryEvent projector**, document render queue (`12-documents-templates.md`), notification delivery, periodic ERP identity re-match (portal's `identity-rematch` cron pattern, `05-users-auth.md`), recurring-order generation (Phase 2). Maintain `scripts/herbe-service-cron.sh` + `scripts/CRON-HANDOFF.md` like both siblings.
