...
Portal-side scope is specified for the portal dev team in `herbe-portal/docs/superpowers/specs/2026-07-04-service-modules-design.md` — **v2.1 (2026-07-06), develop-ready, handed to the portal team on their Bitbucket mainline**: frozen `/api/ext/v1` contract v1.1, `service_connection_config`, QR resolver route, ETA view, feedback module, signing descriptor with full field list, quote-send trigger API, notification ownership, the scope-helper extraction prerequisite, and **Appendix A — client DTOs with per-field ERP provenance** from the round-5 register verification (`17-erp-register-reference.md`), incl. the indicative-prices disclaimer rule, the `erpProcessed` timeline event, and the `RLinkVc`-first invoice cross-link. Tenants without portal: customer touchpoints are the emailed report PDF and the on-site canvas signature — the portal is the upgrade path, not a competing service UI.
