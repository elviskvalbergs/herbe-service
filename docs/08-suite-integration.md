...
Status: v0.6 (2026-07-07, round 5: **"sign-off"** adopted as the umbrella term for the customer-approval/signature triggers POR-2/6/7 (§4); shadow-activity Kanban reframed as existing calendar board configuration — columns filter on `(ActType, ActState)` (`lib/pipeline`), states are service-defined and service-seeded, so no new calendar build is needed for the core Kanban (§3)). Previous: v0.5 (2026-07-07, round 4: **reduced portal integration** — owner decision: the portal service module develops independently and must stand on its own; direct service↔portal integration cut to the worksheet- and quotation-approval triggers plus tentative label/QR, everything else via the ERP; the wider `/api/ext/v1` contract unfrozen — future, re-cut when the portal module settles. Also corrected against sibling source: cron-reuse row, push-group diagram, Kanban field names, design-system repo status). Previous: v0.4 (2026-07-05, round 3: notification-ownership table, QR resolver routing, customer-visible status mapping, `/api/ext` rate-limit line); v0.3 — owner decision applied: **the portal is the only customer-facing surface**; service ships no customer pages. Tier framing, Kanban delegation, Smart Booking intake and the activity-vessel signing fallback folded in from the audit line; `/api/ext` scoping and portal-side implementation notes from review round 2. Written from code-level review of both sibling repos (calendar `main`-merge b57fcbd, portal v1.0.24).
...
**Service-aware calendar features (new work in herbe-calendar, phased with service Phase 2–3; raised as CAL-6…CAL-8 + CAL-5 in `13-suite-change-requests.md`):**
...
## 4. Portal ↔ service: customer surface, sign-off, invoices
...
**Reduced direct integration — owner decision, 2026-07-07.** The portal service module is being developed **independently** on the portal side and must stand on its own, with limited functionality, even without herbe.service; what exactly it becomes is still moving. The **only** direct service↔portal coupling in the near term is the **sign-off** API calls (customer sign-off) — the confirmation/approval-trigger calls; nothing reads across the wire:

1. **Worksheet sign-off trigger** — API endpoints for triggering worksheet sign-off (the sign-off flow below, `POST /api/ext/v1/worksheets/{id}/confirm`).
2. **Quotation sign-off trigger** — API endpoints for triggering quotation sign-off (quote lands in `QTVc`, the sign-off trigger is POR-6; the decision flows back through `QTVc`).
3. **Document sign-off trigger (POR-7)** — the token-authenticated "send delivery-confirmation / signing request" call for customer sign-off, carrying only a reference (`activityId`, optional `documentId`), never document content.
4. **Possibly label/QR** — the portal side of the QR resolver (§4a) stays on the table, unconfirmed.

**The ERP is the connecting piece for all actual data.** The portal reads service data — invoices via `IVVc`, quotes via `QTVc`, bookings/activities via `ActVc`, and anything the wider module surfaces — **from the ERP, never from herbe.service**. The three sign-off triggers above only carry a reference across the wire to fire a notification or fast-path a sync; the underlying data always travels service → ERP → portal via the portal's own ERP sync. **Portal reading data directly from herbe.service is a future step**, taken once the portal service module's shape settles. The wider customer window sketched in earlier rounds (equipment registry + history, request intake, order tracking incl. the "technician on the way" ETA view, approved-worksheet read-out, satisfaction feedback) is the **portal team's own scope**; herbe.service adapts once its shape settles.
...
**Sign-off**: the customer **sign-off** family (POR-2/6/7) covers worksheet reports, quotations and compliance documents. Where a tenant wants formal digital signature instead of/in addition to the on-site canvas signature — `SigningModuleDescriptor` for the worksheet report, `onAllSigned` → `POST /api/ext/v1/worksheets/{id}/confirm`, which writes `CustomerConfirmation` on the worksheet (`02-data-model.md`) and attaches the signed file to the booking's ERP activity where WebExcellentAPI allows. The `ActVc` **activity-vessel** route through the portal's delivery-confirmation flow remains a config-level fallback for compliance documents (POR-2, `12-documents-templates.md`); without any portal, sign-off is the on-site signature.

**Technical contract (reduced, owner 2026-07-07):**
- **Near-term (service Phase 2)** — only the **sign-off** triggers (customer sign-off): `POST /api/ext/v1/worksheets/{id}/confirm` (worksheet sign-off trigger; payload per the sign-off flow above), the quotation sign-off trigger pair with the portal (service→portal send-for-sign-off per POR-6; the accept/reject decision returns through `QTVc`), and the document sign-off trigger (POR-7; reference-only, a latency optimization over the tier-0 poll). Bearer-token auth and rate limiting apply from day one (429 + `Retry-After`; calendar's `lib/rateLimit.ts` pattern).
- **Tentative** — `labelId=` QR resolution for the portal side of §4a, if the label integration is confirmed.
- **Future (re-cut when the portal module settles; no longer frozen)** — the read API: `GET /api/ext/v1/…` service items (+ tree paths), orders (detail incl. booking slots, `enRoute` flag, ETA), approved worksheets, history; `POST /api/ext/v1/requests` (idempotency key required); `POST /api/ext/v1/orders/{id}/feedback`. List endpoints take `customerCodes=` + `after=<changeSeq>`.
- **Customer-visible order status** (kept as the working draft for the future contract — the internal 8-state machine is never exposed verbatim): `New/Accepted → received`, `Planned → scheduled` (+ slot), `In progress/Paused → in progress` (+ `enRoute` flag), `Work done/Confirmed → work done`, `Invoiced/Closed → completed`, `Cancelled → cancelled`.

**§4a — QR labels (one sticker, two audiences).** One physical label per service item node encodes **one resolver URL** carrying the node's stable `labelId` (`02-data-model.md`); routing is by login: technician with the app → service item card (F6); portal customer → portal `service-items/[id]` (tentative — item 3 of the reduced integration, resolved via `labelId=`); unknown visitor → login choice; unknown/unscoped label → friendly "not available", no data leak. **The URL scheme is fixed in Phase 2 before the first label is printed** — stickers outlive software; the portal route joins if/when the label integration is confirmed.
...
| Work done + report ready (+ sign-off invite when sign-off enabled) | herbe.portal | portal `service_work_done` (+ existing `signing_invite`) | portal order detail |
