...
Portal-side scope is specified for the portal dev team in `herbe-portal/docs/superpowers/specs/2026-07-04-service-modules-design.md` — **v2.0 (2026-07-06), a single integrated document** superseding the original + addendum: frozen `/api/ext/v1` contract v1.1, `service_connection_config`, QR resolver route, ETA view, feedback module, signing descriptor with full field list, quote-send trigger API, notification ownership, and the scope-helper extraction prerequisite. Tenants without portal: customer touchpoints are the emailed report PDF and the on-site canvas signature — the portal is the upgrade path, not a competing service UI.
