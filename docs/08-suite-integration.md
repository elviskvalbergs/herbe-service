...
**Signoff (order-level)**: the customer **signoff** family (POR-2/6/7) covers the order service report, quotations and compliance documents. Signoff is on the **order**, not the individual worksheet: once all the order's worksheets are manager-approved, the portal requests the **combined order report** (`GET /api/ext/v1/orders/{id}/report` — all approved worksheets rendered as one document) and presents it. Where a tenant wants formal digital signature instead of/in addition to the on-site canvas signature — `SigningModuleDescriptor` for the order report, `onDecision` → `POST /api/ext/v1/orders/{id}/confirm`, which writes `CustomerConfirmation` on the order (`02-data-model.md`) and attaches the signed file to the order's ERP activity (orderShadow / `SVOVc`) where WebExcellentAPI allows. **The call is resendable**: a `rejected` decision keeps the order open — the office adds a booking + worksheet, and the portal re-requests the (regenerated) report and signoff. Confirmation is independent of the worksheet-level ERP push: parts/time are accounted on manager approval regardless, so a not-yet-confirmed order never strands consumed stock/time. The `ActVc` **activity-vessel** route through the portal's delivery-confirmation flow remains a config-level fallback for compliance documents (POR-2, `12-documents-templates.md`); without any portal, signoff is the on-site signature captured against the same order report.

**Technical contract (reduced, owner 2026-07-07):**
- **Near-term (service Phase 1)** — only the **signoff** triggers (customer signoff): `POST /api/ext/v1/orders/{id}/confirm` (order signoff trigger, resendable; carries the decision `confirmed | rejected` + optional `reason`; payload per the signoff flow above) and its companion `GET /api/ext/v1/orders/{id}/report` (the combined order report the portal presents), the quotation signoff trigger pair with the portal (service→portal send-for-signoff per POR-6; the accept/reject decision returns through `QTVc`), and the document signoff trigger (POR-7; reference-only, a latency optimization over the tier-0 poll). Bearer-token auth and rate limiting apply from day one (429 + `Retry-After`; calendar's `lib/rateLimit.ts` pattern).
- **Committed (service Phase 2, with the customer-experience work)** — satisfaction feedback write-back, **order-level** (a separate axis from confirmation): rating + comment arrive either as an optional `feedback` block on the order-signoff call `POST /api/ext/v1/orders/{id}/confirm` (rated with the signoff) or via a standalone `POST /api/ext/v1/orders/{id}/feedback` (later survey link) — both idempotent. Feeds per-technician/customer/period reporting (owner 2026-07-08 — **direct API, not ERP-mediated**; pulled out of the future/re-cut bucket).
