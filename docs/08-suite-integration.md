...
3. **Possibly label/QR** — the portal side of the QR resolver (§4a) stays on the table, unconfirmed.

**Everything else flows through the ERP as middleman** — invoices via `IVVc`, quotes via `QTVc`, bookings/activities via `ActVc` — with no other direct integration for now. The wider customer window sketched in earlier rounds (equipment registry + history, request intake, order tracking incl. the "technician on the way" ETA view, approved-worksheet read-out, satisfaction feedback) is the **portal team's own scope**; herbe.service adapts once its shape settles.
...
- **Near-term (service Phase 2)** — only the approval triggers: `POST /api/ext/v1/worksheets/{id}/confirm` (worksheet-approval trigger; payload per the signoff flow above) and the quotation-approval trigger pair with the portal (service→portal send-for-approval per POR-6; the accept/reject decision returns through `QTVc`). Bearer-token auth and rate limiting apply from day one (429 + `Retry-After`; calendar's `lib/rateLimit.ts` pattern).
