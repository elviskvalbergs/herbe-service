...
Status: v0.7 (2026-07-07) — roadmap phase renumber: old Phase 1 and Phase 2 merged into one Phase 1; old Phase 3→2, Phase 4→3. Dispatch board, signoff/`/api/ext` near-term contract, QR URL-scheme freeze, person-link import and the erp-core copy-first fallback are now Phase 1; Smart Booking intake and portal-side signing now Phase 2; C4 availability feed now Phase 3. Previous: v0.6 (2026-07-07, round 5: **"signoff"** adopted as the umbrella term for the customer-approval/signature triggers POR-2/6/7 (§4); shadow-activity Kanban reframed as existing calendar board configuration — columns filter on `(ActType, ActState)` (`lib/pipeline`), states are service-defined and service-seeded, so no new calendar build is needed for the core Kanban (§3)). Previous: v0.5 (2026-07-07, round 4: **reduced portal integration** — owner decision: the portal service module develops independently and must stand on its own; direct service↔portal integration cut to the worksheet- and quotation-approval triggers plus tentative label/QR, everything else via the ERP; the wider `/api/ext/v1` contract unfrozen — future, re-cut when the portal module settles. Also corrected against sibling source: cron-reuse row, push-group diagram, Kanban field names, design-system repo status). Previous: v0.4 (2026-07-05, round 3: notification-ownership table, QR resolver routing, customer-visible status mapping, `/api/ext` rate-limit line); v0.3 — owner decision applied: **the portal is the only customer-facing surface**; service ships no customer pages. Tier framing, Kanban delegation, Smart Booking intake and the activity-vessel signing fallback folded in from the audit line; `/api/ext` scoping and portal-side implementation notes from review round 2. Written from code-level review of both sibling repos (calendar `main`-merge b57fcbd, portal v1.0.24).
