...
**Signoff — two scopes, portal-set method**: signoff is configured on two independent axes (`02-data-model.md`). **Scope** = `require_worksheet_confirmation` (on-device, per-worksheet — captured by the technician at `Done`, needs no customer-facing interface, so it's the **portal-less baseline**) and/or `require_order_signoff` (order-level, customer-facing). **Method** = `confirm | sign | digital_sign`, a herbe.portal setting (the `SigningModuleDescriptor`); only the portal path can escalate to a qualified `digital_sign` (Smart-ID / Mobile-ID). The order-level path (this section, POR-2/6/7) covers the order service report, quotations and compliance documents: once all the order's worksheets are manager-approved, the portal requests the **combined order report** (`GET /api/ext/v1/orders/{id}/report` — all approved worksheets rendered as one document) and presents it, then `onDecision` → `POST /api/ext/v1/orders/{id}/confirm`, which writes `OrderSignoff` on the order (`02-data-model.md`) and attaches the signed file to the order's ERP activity (orderShadow / `SVOVc`) where WebExcellentAPI allows. **The call is resendable**: a `rejected` decision keeps the order open — the office adds a booking + worksheet, and the portal re-requests the (regenerated) report and signoff. Confirmation is independent of the worksheet-level ERP push: parts/time are accounted on manager approval regardless, so a not-yet-confirmed order never strands consumed stock/time. The no-module fallback for compliance documents (POR-2, `12-documents-templates.md`) attaches the document to the **`orderShadow`** activity, which the portal's delivery-confirmation flow reads (no dedicated vessel purpose); with no portal, the order-report on-site signature (or the per-worksheet confirmation) is the baseline.

**Technical contract (reduced, owner 2026-07-07):**
- **Phase 1 (framework only)** — the `/api/ext/v1` shell: hashed scoped bearer tokens per company connection, admin minting UI, auth, and rate limiting from day one (429 + `Retry-After`; calendar's `lib/rateLimit.ts` pattern). The signoff / report / feedback endpoint *shapes* are designed against this contract in Phase 1, but the endpoints themselves go live in Phase 2 — they write Phase-2 entities (`OrderSignoff`, `CustomerFeedback`), so placing them earlier would be a phase-crash.
- **Phase 2 (customer experience)** — the customer **signoff** + feedback endpoints, all committed direct portal→service (not ERP-mediated): `GET /api/ext/v1/orders/{id}/report` (the combined order report the portal presents), `POST /api/ext/v1/orders/{id}/confirm` (order signoff, resendable; `confirmed | rejected` + optional `reason` + optional `feedback {rating, comment}`, writing `OrderSignoff` on the order), the standalone `POST /api/ext/v1/orders/{id}/feedback` (later survey, order-level, a separate axis from confirmation), the quotation signoff trigger pair (POR-6; the accept/reject decision returns through `QTVc`), and the document signoff trigger (POR-7; reference-only). Feeds per-technician/customer/period reporting.
