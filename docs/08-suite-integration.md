...
- **Near-term (service Phase 2)** — only the confirmation/approval triggers: `POST /api/ext/v1/worksheets/{id}/confirm` (worksheet-approval trigger; payload per the signoff flow above), the quotation-approval trigger pair with the portal (service→portal send-for-approval per POR-6; the accept/reject decision returns through `QTVc`), and the signing-request trigger (POR-7; reference-only, a latency optimization over the tier-0 poll). Bearer-token auth and rate limiting apply from day one (429 + `Retry-After`; calendar's `lib/rateLimit.ts` pattern).
