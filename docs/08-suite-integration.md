...
- **Committed (service Phase 2, with the customer-experience work)** — satisfaction feedback write-back, **worksheet-level** (the worksheet is what the customer signs off): rating + comment arrive either as an optional `feedback` block on the signoff call `POST /api/ext/v1/worksheets/{id}/confirm` (rated with the signature) or via a standalone `POST /api/ext/v1/worksheets/{id}/feedback` (later survey link) — both idempotent. Rolls up to order/customer/period for reporting (owner 2026-07-08 — **direct API, not ERP-mediated**; pulled out of the future/re-cut bucket).
