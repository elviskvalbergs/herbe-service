...
Status: v0.8 (2026-07-08) — satisfaction feedback write-back (`POST /api/ext/v1/orders/{id}/feedback`) confirmed as a **committed direct portal→service call** (owner: direct API, not ERP-mediated), pulled out of the future read-API bucket into the §4 technical contract (Phase 2). Previous: v0.7 (2026-07-07) — roadmap phase renumber: old Phase 1 and Phase 2 merged into one Phase 1; old Phase 3→2, Phase 4→3. Dispatch board, signoff/`/api/ext` near-term contract, QR URL-scheme freeze, person-link import and the erp-core copy-first fallback are now Phase 1; Smart Booking intake and portal-side signing now Phase 2; C4 availability feed now Phase 3. Previous: v0.6 (2026-07-07, round 5: **"signoff"** adopted as the umbrella term for the customer-approval/signature triggers POR-2/6/7 (§4); shadow-activity Kanban reframed as existing calendar board configuration — columns filter on `(ActType, ActState)` (`lib/pipeline`), states are service-defined and service-seeded, so no new calendar build is needed for the core Kanban (§3)). Previous: v0.5 (2026-07-07, round 4: **reduced portal integration** — owner decision: the portal service module develops independently and must stand on its own; direct service↔portal integration cut to the worksheet- and quotation-approval triggers plus tentative label/QR, everything else via the ERP; the wider `/api/ext/v1` contract unfrozen — future, re-cut when the portal module settles. Also corrected against sibling source: cron-reuse row, push-group diagram, Kanban field names, design-system repo status). Previous: v0.4 (2026-07-05, round 3: notification-ownership table, QR resolver routing, customer-visible status mapping, `/api/ext` rate-limit line); v0.3 — owner decision applied: **the portal is the only customer-facing surface**; service ships no customer pages. Tier framing, Kanban delegation, Smart Booking intake and the activity-vessel signing fallback folded in from the audit line; `/api/ext` scoping and portal-side implementation notes from review round 2. Written from code-level review of both sibling repos (calendar `main`-merge b57fcbd, portal v1.0.24).
...
1. **Worksheet signoff trigger** — API endpoints for triggering worksheet signoff (the signoff flow below, `POST /api/ext/v1/worksheets/{id}/confirm`).
...
**Signoff**: the customer **signoff** family (POR-2/6/7) covers worksheet reports, quotations and compliance documents. Where a tenant wants formal digital signature instead of/in addition to the on-site canvas signature — `SigningModuleDescriptor` for the worksheet report, `onAllSigned` → `POST /api/ext/v1/worksheets/{id}/confirm`, which writes `CustomerConfirmation` on the worksheet (`02-data-model.md`) and attaches the signed file to the booking's ERP activity where WebExcellentAPI allows. The `ActVc` **activity-vessel** route through the portal's delivery-confirmation flow remains a config-level fallback for compliance documents (POR-2, `12-documents-templates.md`); without any portal, signoff is the on-site signature.

**Technical contract (reduced, owner 2026-07-07):**
- **Near-term (service Phase 1)** — only the **signoff** triggers (customer signoff): `POST /api/ext/v1/worksheets/{id}/confirm` (worksheet signoff trigger; payload per the signoff flow above), the quotation signoff trigger pair with the portal (service→portal send-for-signoff per POR-6; the accept/reject decision returns through `QTVc`), and the document signoff trigger (POR-7; reference-only, a latency optimization over the tier-0 poll). Bearer-token auth and rate limiting apply from day one (429 + `Retry-After`; calendar's `lib/rateLimit.ts` pattern).
- **Committed (service Phase 2, with the customer-experience work)** — satisfaction feedback write-back: the portal calls `POST /api/ext/v1/orders/{id}/feedback` (idempotent) with rating + comment, so per-technician/period satisfaction reporting has data (owner 2026-07-08 — **direct API, not ERP-mediated**; the one endpoint pulled out of the future/re-cut bucket).
