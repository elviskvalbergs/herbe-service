...
| **Office shell** | Service manager (= dispatcher — one role, `05-users-auth.md`), back office, admin | Desktop-first responsive | Left sidebar (portal `AdminShell` pattern: collapsible, mobile drawer). Sections: Dispatch(P2) · Orders · Worksheets · Customers · Service items · Stock(P2) · Reports(P3) · Settings/Admin. |
...
| O9 | **Checklist template builder** | 2 | service manager | Sections, field types, required rules, bounds; versioning; assignment by item/work type. Phase 1 ships seeded fixed templates managed as data, no builder UI. |
| O10 | **Reports** | 3 | service manager, back office | Utilization, first-time-fix, MTTR, revenue/technician (ERP-priced). |
