...
| A4 | ERP connection | Adapter choice, credentials, capability probe result (incl. WebExcellentAPI) with the **service/contracts document version gate**: a version-match pattern (regex, e.g. `8.5.6*` — suite default, per-connection override) matched against the `systemversion` on the REST `<data>` root, shown read-only alongside the derived on/off flag; it gates service + contracts document/attachment/activity support, while the general WebExcellentAPI actions (invoice/order PDFs, `windowactions`, `getrecordlinks`) are presence-gated only (`04-erp-sync.md` API tiers). Register/field map versions, **activity-purpose map** (per-purpose ActVc type/symbol, six purposes per `04-erp-sync.md`: `booking`, `worksheetShadow` (Phase 1), `intake`, `workSegment` (P2), `documentVessel`, `historyImport`), invoice back-link field, timezone, poll cadences, maintenance window; **transformations editor (declarative maps + JS hooks) and settings import/export** (P2, `04-erp-sync.md`); **`/api/ext` service API tokens** (mint/revoke per company connection, P2). |
