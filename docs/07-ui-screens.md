...
| O3 | **Worksheet approval queue** | 1 | service manager (+ team lead per tenant flag, `05-users-auth.md`) | Worksheets in `Done`, oldest first — **crew jobs show as N worksheets grouped by `crewGroupId`** (round 6, `02-data-model.md`; grouping is Phase 1, owner 2026-07-07) rather than one row per job — plus the **orders in `Work done`** ("job done" review queue); approve/reject per item or bulk. |
...
| A4 | ERP connection | Adapter choice, credentials, capability probe result (incl. WebExcellentAPI) with the **service/contracts document version gate** (a version-match pattern/regex for service + contracts document/attachment/activity support, e.g. `8.5.6*` — suite default, per-connection override; matched against the `systemversion` the REST `<data>` root returns, shown read-only alongside the derived on/off flag; the general WebExcellentAPI actions for invoices/orders are presence-gated only — `04-erp-sync.md` API tiers), register/field map versions, **activity-purpose map** (per-purpose ActVc type/symbol, six purposes per `04-erp-sync.md`: `booking`, `worksheetShadow` (ships Phase 1, minimal — round 7), `intake`, `workSegment` (P2; subsumes the earlier time-entry mirror), `documentVessel`, `historyImport`), invoice back-link field, timezone, poll cadences, maintenance window; **transformations editor (declarative maps + JS hooks) and settings import/export** (P2, `04-erp-sync.md`); **`/api/ext` service API tokens** (mint/revoke per company connection, P2). |
