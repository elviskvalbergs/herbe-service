...
|---|---|---|---|---|
| F1 | **Today** (landing) | 1 | Ordered list of today's bookings: time, customer, site, order summary, status chips; overdue/unfinished carried over from yesterday. Actions: open job, call contact, navigate. Pull-to-refresh = delta sync. | full |
| F2 | **My jobs** | 1 | Day/week calendar + list of own bookings (simple; not the dispatch board). Filter: open/done. | full |
...
| F4 | **Worksheet execution** | 1 | Tabbed: **Work** (description, fault/cause/remedy) · **Parts** (rows: item search/scan(P2), qty, stock location) · **Time** (start/stop timer + manual entries, work/travel) · **Checklist** (template-driven form, pass/fail bounds) · **Photos** (camera, before/after tag) . Footer: status stepper `In progress → Done`. | full |
| F5 | **Signature capture** | 1 | Summary the customer sees (customer-visible rows, no prices if role-gated), name field, canvas signature, geo+timestamp captured. Locks content (revision rule in `02-data-model.md`). | full |
...
| F7 | **Customer / site card** | 1 | Addresses with navigate, contacts with call, open orders, service items at site, notes. Create-in-field (tenant-configurable): new customer/site/service item with duplicate check on sync. | full |
...
| O5 | **Dispatch board** | 2 | dispatcher | Day/week × technician rows; drag-and-drop bookings; unassigned-work pool; conflicts flagged inline; self-assignment pool toggle. Realtime-ish via delta polling. |
...
| A8 | Migrations/ops | Build-time + admin-run migration runner (portal pattern), health endpoint status. |
...
5. **Re-plan (P2)**: O5 drag booking → if worksheet `Draft/Assigned`, assignee follows; else manager confirms reassignment. Tech gets notification + F10 entry.
