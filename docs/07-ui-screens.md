...
| A4 | ERP connection | Adapter choice, credentials, capability probe result (incl. WebExcellentAPI) with the **version gate** (per-capability minimum ERP/Books version thresholds — suite defaults, per-connection override; installation version auto-read over REST with manual-entry fallback; derived capability flags shown read-only — `04-erp-sync.md` API tiers), register/field map versions, **activity-purpose map** (per-purpose ActVc type/symbol, six purposes per `04-erp-sync.md`: `booking`, `worksheetShadow` (ships Phase 1, minimal — round 7), `intake`, `workSegment` (P2; subsumes the earlier time-entry mirror), `documentVessel`, `historyImport`), invoice back-link field, timezone, poll cadences, maintenance window; **transformations editor (declarative maps + JS hooks) and settings import/export** (P2, `04-erp-sync.md`); **`/api/ext` service API tokens** (mint/revoke per company connection, P2). |
