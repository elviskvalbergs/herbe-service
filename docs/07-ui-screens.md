...
Status: v0.4 (2026-07-06, owner round 6: worksheet model reverted to one-per-technician — F4/F5/O3 crew wording updated, no shared-worksheet attribution UI needed). Previous: v0.3 (2026-07-06, owner round 4: booking-less jobs in F1/F2/F7, prepared-ahead worksheets + manual confirm in O2/O3, charge types + re-sign decision in O4). Previous: v0.2 (2026-07-05, round 3: O4 media gate + correction entry, O11 unlinked-activity list). Originally added by the spec review — the v0.1 spec had no UI coverage. Wireframes per screen are a Phase 0/1 design task; this doc fixes the inventory, navigation, and flow contracts so design and development can start.
...
| O3 | **Worksheet approval queue** | 1 | manager | Worksheets in `Done`, oldest first — **crew jobs show as N worksheets grouped by `crewGroupId`** (round 6, `02-data-model.md`) rather than one row per job — plus the **orders in `Work done`** ("job done" review queue); approve/reject per item or bulk. |
