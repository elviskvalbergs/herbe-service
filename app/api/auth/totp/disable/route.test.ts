// app/api/auth/totp/disable/route.test.ts
//
...
// secret). No 403-non-admin case: this route is deliberately self-service
// with no role gate (only the admin-gated enroll routes can ever turn MFA
// on in the first place — see the brief).
...
describe('POST /api/auth/totp/disable', () => {
  ...
  it('rejects a wrong code with 400 invalid_code', async () => {
    const user = await makeUser('totp-disable-wrong', 'admin-wrong@herbe-service.test', 'admin')
  ...
  it('disables MFA with a correct TOTP code: clears the secret, flips mfaEnabled false, bumps sessionVersion', async () => {
    const user = await makeUser('totp-disable-ok', 'admin-ok@herbe-service.test', 'admin')
    const enrolled = await enrollTotp(db, user.id, user.email)
    await confirmTotpEnrollment(db, user.id, currentCodeFor(enrolled.secretBase32, user.email))
    authMock.mockResolvedValue(sessionFor(user))
  ...
  it('disables MFA with a correct recovery code too', async () => {
    const user = await makeUser('totp-disable-recovery', 'admin-recovery@herbe-service.test', 'admin')
    const enrolled = await enrollTotp(db, user.id, user.email)
    await confirmTotpEnrollment(db, user.id, currentCodeFor(enrolled.secretBase32, user.email))
    authMock.mockResolvedValue(sessionFor(user))

    const { POST } = await import('./route')
    const res = await POST(makeRequest({ code: enrolled.recoveryCodes[0] }))
    const body = await res.json()
