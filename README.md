...
6. **Integrate, don't duplicate — but stay a standalone app.** herbe.calendar owns time (team calendars, Kanban boards, Smart Booking); herbe.portal owns the customer's document/payment/signing window and grows the service modules; herbe.service owns field work, exposes the service API they read, and keeps tokenized customer links so the loop closes even without the siblings ([08-suite-integration.md](docs/08-suite-integration.md); asks on the sibling teams in [13-suite-change-requests.md](docs/13-suite-change-requests.md)).
